...
> **COMPLETED 2026-08-19.** All 7 tasks landed; the 12-module `streamlit/` tree is deployed and
> owned by `TRANSCRIPTION_APP_ROLE`.
>
> **One decision in this plan was WRONG and has been reversed. Do not follow it.**
> The "leave Streamlit unpinned" decision assumed an unpinned runtime resolves a recent version.
> It resolves the **OLDEST supported version, 1.22.0 (2023)**. That hard-broke
> `st.file_uploader` and silently degraded `st.download_button` (needs 1.26), `hide_index` and
> `st.column_config` (1.23), `st.scatter_chart` (1.27), `st.fragment` (1.37) and chart
> `x_label`/`y_label` (1.39). `streamlit/environment.yml` now pins **1.52.2** and
> `lint_dashboard.py` fails the build if the pin is missing. See known-issues §7.
>
> Other drift, corrected in the shipped code:
> - `secondaryBackgroundColor` is **`#E8F4FB`**, not the brand `#F0F7FB` — the brand tint is
>   visually indistinguishable from Streamlit's default `#F0F2F6`, so it applied but could not
>   be seen.
> - `linkColor` and `[theme.sidebar]` were **dropped**; the shipped `config.toml` has 6 keys.
> - `tab_browse` is **`render(session, df)`**, not `render(df)` as specified here.
> - The entrypoint is ~195 lines, not the predicted ~130.
...
| Streamlit version | ~~**Leave unpinned** — no `environment.yml`; the existing `st.fragment` fallback covers older runtimes~~ **REVERSED 2026-08-19 — see correction below** |
